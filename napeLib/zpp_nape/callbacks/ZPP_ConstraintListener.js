...
export default class ZPP_ConstraintListener extends ZPP_Listener {
	...
	_hx_constructor(options,event,handler) {
		...
		this.type = ZPP_Flags.id_ListenerType_CONSTRAINT;
	...
	swapEvent(newev) {
		if(newev != ZPP_Flags.id_CbEvent_WAKE && newev != ZPP_Flags.id_CbEvent_SLEEP && newev != ZPP_Flags.id_CbEvent_BREAK) {
			throw haxe_Exception.thrown("Error: ConstraintListener event must be either WAKE or SLEEP only");
		}
		this.removedFromSpace();
		this.event = newev;
		this.addedToSpace();
