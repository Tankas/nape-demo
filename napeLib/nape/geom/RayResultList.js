import ZPP_RayResultList from '../../zpp_nape/util/ZPP_RayResultList.js';
import RayResultIterator from './RayResultIterator.js';
export default class RayResultList {
	constructor() {
		this.zpp_inner = null;
	...
	push(obj) {
		if(this.zpp_inner.immutable) {
			throw haxe_Exception.thrown("Error: " + "RayResult" + "List is immutable");
		}
	...
	copy(deep) {
		...
		let ret = new RayResultList();
		this.zpp_inner.valmod();
		let _g = RayResultIterator.get(this);
		while(true) {
			...
			let tmp1;
			if(deep) {
				throw haxe_Exception.thrown("Error: " + "RayResult" + " is not a copyable type");
			} else {
				tmp1 = i;
			}
			ret.push(tmp1);
	...
	merge(xs) {
		...
		xs.zpp_inner.valmod();
		let _g = RayResultIterator.get(xs);
		while(true) {
			_g.zpp_inner.zpp_inner.valmod();
			let _this = _g.zpp_inner;
			_this.zpp_inner.valmod();
	...
	foreach(lambda) {
		...
		this.zpp_inner.valmod();
		let it = RayResultIterator.get(this);
		while(true) {
