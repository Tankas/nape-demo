...
import ConvexResultIterator from './ConvexResultIterator.js';
export default class ConvexResultList {
	constructor() {
		this.zpp_inner = null;
	...
	push(obj) {
		if(this.zpp_inner.immutable) {
			throw haxe_Exception.thrown("Error: " + "ConvexResult" + "List is immutable");
		}
	...
	copy(deep) {
		...
		let ret = new ConvexResultList();
		this.zpp_inner.valmod();
		let _g = ConvexResultIterator.get(this);
		while(true) {
			...
			let tmp1;
			if(deep) {
				throw haxe_Exception.thrown("Error: " + "ConvexResult" + " is not a copyable type");
			} else {
				tmp1 = i;
			}
			ret.push(tmp1);
	...
	merge(xs) {
		...
		xs.zpp_inner.valmod();
		let _g = ConvexResultIterator.get(xs);
		while(true) {
			_g.zpp_inner.zpp_inner.valmod();
			let _this = _g.zpp_inner;
			_this.zpp_inner.valmod();
	...
	foreach(lambda) {
		...
		this.zpp_inner.valmod();
		let it = ConvexResultIterator.get(this);
		while(true) {
