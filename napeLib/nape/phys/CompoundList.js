...
export default class CompoundList {
	constructor() {
		this.zpp_inner = null;
	...
	copy(deep) {
		...
		let ret = new CompoundList();
		this.zpp_inner.valmod();
		let _g = CompoundIterator.get(this);
		while(true) {
			...
			let tmp1;
			if(deep) {
				throw haxe_Exception.thrown("Error: " + "Compound" + " is not a copyable type");
			} else {
				tmp1 = i;
			}
			ret.push(tmp1);
	...
	merge(xs) {
		...
		xs.zpp_inner.valmod();
		let _g = CompoundIterator.get(xs);
		while(true) {
			_g.zpp_inner.zpp_inner.valmod();
			let _this = _g.zpp_inner;
			_this.zpp_inner.valmod();
	...
	foreach(lambda) {
		...
		this.zpp_inner.valmod();
		let it = CompoundIterator.get(this);
		while(true) {
