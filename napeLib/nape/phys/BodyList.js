...
export default class BodyList {
	...
	copy(deep) {
		...
		let ret = new BodyList();
		this.zpp_inner.valmod();
		let _g = BodyIterator.get(this);
		while(true) {
			...
			let tmp1;
			if(deep) {
				throw haxe_Exception.thrown("Error: " + "Body" + " is not a copyable type");
			} else {
				tmp1 = i;
			}
			ret.push(tmp1);
