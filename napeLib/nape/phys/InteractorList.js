...
import InteractorIterator from './InteractorIterator.js';
export default class InteractorList {
	constructor() {
		this.zpp_inner = null;
	...
	push(obj) {
		if(this.zpp_inner.immutable) {
			throw haxe_Exception.thrown("Error: " + "Interactor" + "List is immutable");
		}
	...
	copy(deep) {
		...
		let ret = new InteractorList();
		this.zpp_inner.valmod();
		let _g = InteractorIterator.get(this);
		while(true) {
			...
			let tmp1;
			if(deep) {
				throw haxe_Exception.thrown("Error: " + "Interactor" + " is not a copyable type");
			} else {
				tmp1 = i;
			}
			ret.push(tmp1);
	...
	merge(xs) {
		...
		xs.zpp_inner.valmod();
		let _g = InteractorIterator.get(xs);
		while(true) {
			_g.zpp_inner.zpp_inner.valmod();
			let _this = _g.zpp_inner;
			_this.zpp_inner.valmod();
	...
	foreach(lambda) {
		...
		this.zpp_inner.valmod();
		let it = InteractorIterator.get(this);
		while(true) {
