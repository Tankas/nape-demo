...
import ShapeIterator from './ShapeIterator.js';
export default class ShapeList {
	...
	copy(deep) {
		...
		let ret = new ShapeList();
		this.zpp_inner.valmod();
		let _g = ShapeIterator.get(this);
		while(true) {
			...
			let tmp1;
			if(deep) {
				throw haxe_Exception.thrown("Error: " + "Shape" + " is not a copyable type");
			} else {
				tmp1 = i;
			}
			ret.push(tmp1);
