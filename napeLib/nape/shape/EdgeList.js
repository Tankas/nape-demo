import ZPP_EdgeList from '../../zpp_nape/util/ZPP_EdgeList.js';
...
export default class EdgeList {
	...
	push(obj) {
		if(this.zpp_inner.immutable) {
			throw haxe_Exception.thrown("Error: " + "Edge" + "List is immutable");
		}
	...
	copy(deep) {
		...
		let ret = new EdgeList();
		this.zpp_inner.valmod();
		let _g = EdgeIterator.get(this);
		while(true) {
			...
			let tmp1;
			if(deep) {
				throw haxe_Exception.thrown("Error: " + "Edge" + " is not a copyable type");
			} else {
				tmp1 = i;
			}
			ret.push(tmp1);
