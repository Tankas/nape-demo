...
import CbTypeIterator from './CbTypeIterator.js';
export default class CbTypeList {
	constructor() {
		this.zpp_inner = null;
	...
	copy(deep) {
		...
		this.zpp_inner.valmod();
		let _g = CbTypeIterator.get(this);
		while(true) {
			...
			let tmp1;
			if(deep) {
				throw haxe_Exception.thrown("Error: " + "CbType" + " is not a copyable type");
			} else {
				tmp1 = i;
			}
			ret.push(tmp1);
	...
	foreach(lambda) {
		...
		this.zpp_inner.valmod();
		let it = CbTypeIterator.get(this);
		while(true) {
