...
import ContactIterator from './ContactIterator.js';
export default class ContactList {
	...
	push(obj) {
		if(this.zpp_inner.immutable) {
			throw haxe_Exception.thrown("Error: " + "Contact" + "List is immutable");
		}
	...
	copy(deep) {
		...
		this.zpp_inner.valmod();
		let _g = ContactIterator.get(this);
		while(true) {
			...
			let tmp1;
			if(deep) {
				throw haxe_Exception.thrown("Error: " + "Contact" + " is not a copyable type");
			} else {
				tmp1 = i;
			}
			ret.push(tmp1);
	...
	foreach(lambda) {
		...
		this.zpp_inner.valmod();
		let it = ContactIterator.get(this);
		while(true) {
